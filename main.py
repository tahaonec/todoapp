...
class Interface(ScreenManager):
    ...
    def fetching_data(self,dt):
        try:
            keys=store.keys()
            for key in keys:
                layout = BoxLayout(size_hint_y=None, height=dp(80),spacing=dp(10))
                self.ids[key]=layout
                title = Custombtn(background_normal="background.png",font_name="robotolight.ttf",key_name=key,text=self.truncate_string(key,10))
                delete = Custombtn(key_name=key,on_press=self.deleting,background_normal="delete.png", size_hint=(None, None), size=(dp(80), dp(80)))
    ...
    def addItem(self,obj):
        self.popup.dismiss()
        layout=BoxLayout(size_hint_y=None, height=dp(80),spacing=dp(10))
        title=Custombtn(background_normal="background.png",font_name="robotolight.ttf",key_name=self.textInput.text,text=self.truncate_string(self.textInput.text,10))
        delete=Custombtn(background_normal="delete.png",on_press=self.deleting,key_name=self.textInput.text, size_hint=(None,None), size=(dp(80),dp(80)))
    ...
    def show_popup(self):
        ...
        btn=Button(font_name="robotoblack.ttf",text="Submit", background_normal="add.png")
